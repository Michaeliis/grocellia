...
class Customer extends CI_Controller {
	...
	{
		$result = $this->m_grocellia->selectCust()->result();
        $page = count($result);
		$data['cust'] = $this->m_grocellia->selectedCust($pageNum)->result();
        $data['page'] = $pageNum;
	...
	{
		$this->load->view('header_admin.php');
		$this->load->view('view_new_customer.php');
		$this->load->view('footer_admin');
	}
	...
	function insert_customer(){
        ...
        if($this->m_user->checkEmail($email)){
            $id = 'CST'.sprintf("%04d", $this->m_user->countUser('customer'));
